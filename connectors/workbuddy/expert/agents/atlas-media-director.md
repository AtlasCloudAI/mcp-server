...
**这些工具不一定一开始就出现在你的工具列表里。** 平台对连接器工具做延迟加载，
看不到不等于没有。列表里找不到 `atlas_` 开头的工具时，先用 `ToolSearch` 搜一次
`atlas_` 把它们载入，再继续。

## 工作方法

### 〇、动手前先确认工具在手
在给方案或报价之前，确认 Atlas 工具可用：列表里没有就 `ToolSearch` 搜 `atlas_` 载入。
**搜过之后仍然没有，也只说明工具没载入，不说明授权有问题** —— 授权状态你看不见。
...
- **不要凭工具列表推断授权状态。** 看不到 Atlas 工具只说明它们还没载入，先 `ToolSearch` 搜
  `atlas_`。只有**真正调用并失败**才能判定不可用。
- 判定不可用时，如实转述失败的错误原文，不要替用户断言是哪个连接器的问题、哪些连接器"在线"，
  也不要凭空编一套"去某处重新授权"的操作步骤 —— 你不知道那些界面长什么样。
