...
## 图标用在哪几处（2026-09-22 核实）

Codex/OpenAI 提交材料里的 logo 是
`AtlasCloudAI/atlas-cloud-plugin @ v0.6.0 → plugins/atlas-cloud/assets/app-icon.svg`
（`docs/chatgpt-app-submission.json` 的 `app_info.logo_url`）。

**本仓 `connectors/workbuddy/icon.svg` 和它逐字节相同**，连接器市场图标不用换。

四个位置：

| # | 位置 | 状态 |
| --- | --- | --- |
| ① | `connectors/workbuddy/icon.svg` 连接器市场图标 | 已是该 logo |
| ② | `expert/avatars/expert.png` 专家头像 | 薇拉卡通像（见下） |
| ③ | dev 变体 `.mcp.json` 的 `x-workbuddy.icon` 依赖引导卡 | 2026-09-22 补上 `./avatars/atlas-logo.svg` |
| ④ | 开放平台后台草稿 `oc_630b5b5c9e689e22` | 只能网页后台改 |

③ 只存在于 dev 变体：提审版用 `dependencies.connectors`，引导卡的名称/描述/图标**由连接器自身提供**，
不需要在专家包里再声明一次。icon 相对路径基于「该 MCP 声明文件所在目录」，放包根的 `.mcp.json`
就写 `./avatars/xxx.svg`。该字段接受 SVG，不必转 PNG。

**② 2026-09-23 改成了 logo。** 此前这里写的是「建议保持人物肖像」，理由是专家有个人名（薇拉），
人格化角色配人像才符合「角色定位」。**改名之后那个前提没了**——展示名变成「Atlas Cloud 媒体制作专家」，
是品牌+职能，再配一张具体某个人的脸，比两种纯粹方案都别扭。腾讯自家的 `gaokao-advisor` 用人像，
但它的展示名也是「专业高考顾问」配人像，两者一致；我们现在一致的选择是 logo。

头像从 188KB 的生成插画换成 21KB 的 logo PNG（`icon.svg` 按坐标用 PIL 画，4 倍超采样降采样）。

本机没有 cairosvg/rsvg/inkscape。要把这个 SVG 转 PNG 可以直接用 PIL 按坐标画
（圆角矩形 + 一条三次贝塞尔路径，4 倍超采样再降采样），脚本思路见会话记录。
