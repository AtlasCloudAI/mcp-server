...
- 🎬 **動画** (200) — MiniMax H3 Max · MiniMax H3 Fast · Gemini Omni 1.1 Flash · MiniMax H3 · Wan-3.0-Prime · Wan-3.0
- 🎨 **画像** (130) — GPT Image 2.5 Sunburst · GPT Image 2.5 Flare · Seedream v4.7 · MAI-Image-2.6-Flash
...
- 🔊 **音声 (TTS · 音楽 · 音声認識)** (13) — Seed Audio 1.0 · MiniMax Speech 2.6 Turbo · MiniMax Speech 2.6 HD · xAI TTS v1

- 📚 **さらに探す** — [全 450 モデル »](https://www.atlascloud.ai/models?utm_source=github&utm_campaign=mcp-server)
<!-- ATLAS-MODELS:END -->
...
- 🎬 **「この商品写真を 5 秒の広告に変えて」** — Seedance 2.5、Kling 3、Kling Video O3、Veo 3.1 などによる画像から動画の生成
...
その裏側では、モデル探索、モデルごとの動的なパラメータスキーマ（リクエストごとに事前検証され、無効なパラメータはクレジットを消費せずに即座に失敗します。各生成ツールには `dry_run` もあり、送信せずに実際のリクエストボディを確認できます）、メディアアップロード、ワンステップのクイック生成、アカウント残高と使用量、ドキュメント検索が動作しており、これらはすべて標準的な MCP ツールとして提供されています（[利用可能なツール](#利用可能なツール)を参照）。
